      ...
      if (*w == tr->val) {
        if (*(w + 1) == '\0') {
          ...
          if (tr->postfix == (Trie)NULL) {
            ...
            #pragma omp task untied default(shared) depend(in:w) depend(inout:tr[0][3]) final(cutoff_test)
            {
        ...
        if (tr->otherChar == (Trie)NULL) {
          ...
          {
          cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
          #pragma omp task untied default(shared) depend(in:w) depend(inout:tr[0][4]) final(cutoff_test)
          {
...
/* Insert a NON-EMPTY word in a new trie */
{
  ...
  if (*w == '\0')
    ...
    if (*w == '\0') {
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(in:w) final(cutoff_test)
      {
...
void printTheRest(Trie t, SString word) {
  ...
  if (t == NULL) {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:t[0][3]) depend(inout:word) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:t[0][4]) depend(inout:word) final(cutoff_test)
    {
