...
void add_segments(DCEL_segment *n, DCEL_segment *act, DCEL_segment *first,
  ...
  if (visible(direction, n->v1, n->v2, p)) {
    ...
    if (direction == l) {
      ...
      #pragma omp task untied default(shared) depend(out:first) depend(inout:k,k[0][2]) final(cutoff_test)
      {
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(out:first) depend(inout:k,k[0][4]) final(cutoff_test)
      {
