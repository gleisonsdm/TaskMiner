...
void *splay(splay_node **root, long key) {
  ...
  while (temp->father != NULL) {
    if (temp->father->father == NULL) { /* rotate x */
      temp = rotate(temp);
    } else if ((temp->father->left == temp) &&
               (temp->father->father->left == temp->father)) { /* x and y both left children, rotate y, rotate x */
      rotate(temp->father);
      temp = rotate(temp);
    } else if ((temp->father->right == temp) &&
               (temp->father->father->right == temp->father)) { /* x and y both right children, rotate y, rotate x */
      rotate(temp->father);
      temp = rotate(temp);
