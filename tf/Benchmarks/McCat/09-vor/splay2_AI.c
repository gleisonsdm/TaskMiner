...
CHsplay_node *CHfind(CHsplay_node *root, key key) {
  ...
  if (((root->element.key.radius < key.radius) ||
    ...
    #pragma omp task untied default(shared) depend(in:root[0][3],key) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:root[0][2],key) final(cutoff_test)
    {
  ...
  while (temp->father != NULL) {
    if (temp->father->father == NULL) { /* CHrotate x */
      temp = CHrotate(temp);
               ...
               (temp->father->father->left == temp->father)) { /* x and y both left children, CHrotate y, CHrotate x */
      CHrotate(temp->father);
      temp = CHrotate(temp);
    } else if ((temp->father->right == temp) &&
               (temp->father->father->right == temp->father)) { /* x and y both right children, CHrotate y, CHrotate x */
      CHrotate(temp->father);
      temp = CHrotate(temp);
...
void CHtraverse(CHsplay_node *root) {
  ...
  if (root != NULL) {
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:root[0][2]) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:root[0][3]) final(cutoff_test)
    {
