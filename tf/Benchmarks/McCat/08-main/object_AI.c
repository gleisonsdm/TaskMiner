  ...
  if (o->Number_of_Points == 0) /* The simple case */ {
    ...
    while (((tmp) != NULL) && (p == NULL)) {
      if (FPEqual(tmp->P->x, x) &&
          FPEqual(tmp->P->y, y) &&
          FPEqual(tmp->P->z, z)) { /* We found the point in the list */
#ifdef DEBUG_OSYS
    ...
    if (tmp == NULL && p == NULL) { /*Then we haven't found the point in the list so we insert new */
#ifdef DEBUG_OSYS
...
ObjPtr /*  (         double array[][]        ) giver 1 array med alle tal */
  ...
  for (i = 0; i < size; i = i + 3) {
    ...
    #pragma omp task untied default(shared) depend(inout:o)
    {
...
void PrintPoly3s(ObjPtr o) {
  ...
  if (o->Poly3s != NULL) {
    ...
    while (tmp != NULL) {
      for (i = 0; i < 3; i++) {
        printf("PrintPoly3s[%i] = (%.2f, %.2f, %.2f)", i, tmp->P[i]->x, tmp->P[i]->y, tmp->P[i]->z);
...
void PrintPoly4s(ObjPtr o) {
  ...
  if (o->Poly4s != NULL) {
    ...
    while (tmp != NULL) {
      for (i = 0; i < 4; i++) {
        printf("PrintPoly4s[%i] = %.2f, %.2f, %.2f", i, tmp->P[i]->x, tmp->P[i]->y, tmp->P[i]->z);
        printf("-> (%.2f, %.2f, %.2f)\n", tmp->P[i]->tx, tmp->P[i]->ty, tmp->P[i]->tz);
      }
      tmp = tmp->Next;
      j++;
...
void CalcObjectChildren(ObjPtr o,
                        #pragma omp critical
