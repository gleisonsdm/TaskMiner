...
void ApplyGivens(Matrix A, double s, double c, int i, int j, int start, int end) { /* A = G.t()AG, G = Givens(i,j,theta); */
  double t1, t2;
  int k;

  for (k = start; k <= end; k++) { /* A = G.t()*A */
    t1 = A[i][k];
    t2 = A[j][k];
    A[i][k] = c * t1 - s * t2;
    A[j][k] = s * t1 + c * t2;
...
Matrix Jacobi(Matrix A, int bw) {
  ...
  for (i = bw; i >= 2; i--) {     /* Remove one bandwith at a time */
    for (j = 0; j < n - i; j++) { /* Iterate through the diagonal */
