...
void Insert(int n, struct node *t) {
  ...
  if (n > t->val)
    if (t->left == nil)
      ...
      #pragma omp task untied default(shared) depend(inout:t) final(cutoff_test)
      {
      Insert(n, t->left);
  }
   }
    ...
    if (t->right == nil)
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(inout:t[0].right) final(cutoff_test)
      {
      Insert(n, t->right);
#pragma omp critical
taskminer_depth_cutoff--;
...
int Checktree(struct node *p) {
  ...
  if (p->left != nil)
    if (p->left->val <= p->val)
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(in:p) final(cutoff_test)
      {
  ...
  if (p->right != nil)
    if (p->right->val >= p->val)
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(in:p[0].right) final(cutoff_test)
      {
