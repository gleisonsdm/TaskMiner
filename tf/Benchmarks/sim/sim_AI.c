...

{
  ...
  if (type == 1) {
    ...
    #pragma omp task untied default(shared) depend(in:A,B,q) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:A[midi],B[0],q) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:A,B,zero) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:A[midi],B[0],zero) final(cutoff_test)
    {
