...
static void
    set_file_variables(file) register struct file *file;
{
  register char *p;
...
void
    chop_commands(cmds) register struct commands *cmds;
{
  if (cmds != 0 && cmds->command_lines == 0) {
    ...
    for (idx = 0; idx < nlines; ++idx) {
      ...
      p = lines[idx];
      len = strlen(p);
      recursive = (sindex(p, len, "$(MAKE)", 7) != 0 || sindex(p, len, "${MAKE}", 7) != 0);
...
void
    execute_file_commands(file) struct file *file;
{
  register char *p;
...
int
    fatal_error_signal(sig) int sig;
{
  signal(sig, SIG_DFL);
...
void
    delete_child_targets(child) struct child *child;
{
  struct stat st;
  struct dep *d;
  ...
  if (!child->file->precious && stat(child->file->name, &st) == 0 && S_ISREG(st.st_mode) && (time_t)st.st_mtime != child->file->last_mtime) {
  ...
  for (d = child->file->also_make; d != 0; d = d->next)
    if (!d->file->precious)
      if (stat(d->file->name, &st) == 0 && S_ISREG(st.st_mode) && (time_t)st.st_mtime != d->file->last_mtime) {
...
void
    print_commands(cmds) register struct commands *cmds;
{
  register char *s;
