...
void InOrder(HANDLE *h) {
  ...
  if ((h != NIL)) {
    ...
    #pragma omp task untied default(shared) depend(in:l) final(cutoff_test)
    {
    InOrder(l);
    }
     }
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:r) final(cutoff_test)
    {
    InOrder(r);
  }
  #pragma omp taskwait
...

{
  ...
  if ((root->left != NIL)) {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(inout:rl,value) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(inout:rr) final(cutoff_test)
    {
...

{
  ...
  if ((root->left == NIL)) /* <---- 8.7% load penalty */
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(inout:l,val) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(inout:r) final(cutoff_test)
    {
