...
{                                        /*  beginning of outputQuery()  */
  ...
  assert(REPRESENTATION_LENGTH >=
         LENGTH_OF_KEY_VALUE * NUM_OF_KEY_ATTRIBUTES + LENGTH_OF_NON_KEY_VALUE * NUM_OF_NON_KEY_ATTRIBUTES + (NUM_OF_LARGE_ATTRIBUTES - 1) + 1);
...
void initOutputBuffer(FILE *file) { /*  beginning of initOutputBuffer() */
  assert(file);
...
void flushOutputBuffer(void) { /*  beginning of flushOutputBuffer()    */
  assert(outputFile);
