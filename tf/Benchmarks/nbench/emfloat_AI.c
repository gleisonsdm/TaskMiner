...
*/
  ...
  #pragma omp parallel
  #pragma omp single
  for (i = 0; i < arraysize; i++) { /*       LongToInternalFPF(randwc(50000L),&locFPF1); */
    ...
    #pragma omp task untied default(shared) depend(inout:abase[i]) shared(locFPF1,locFPF2)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(inout:bbase[i]) shared(locFPF1,locFPF2)
    {
