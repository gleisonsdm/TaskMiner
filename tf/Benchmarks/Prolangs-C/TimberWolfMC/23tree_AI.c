...
void tdiscard(TNODEPTR *root, int count) {
  ...
  for (i = 1; i <= count; i++) {
    ...
    #pragma omp task untied default(shared) depend(inout:root) shared(node,property,value)
    {
    tmax(root, &node, &value, &property);
    }
     }
    if (node == (TNODEPTR)NULL) {
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(inout:root)
      {
      tdelete(root, value, property);
    }
     }
...
taskminer_depth_cutoff++;
  ...
  if (r->son1->nsons == 0) {
    ...
    if (val <= r->Lval) {
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(in:r[0][2]) final(cutoff_test)
      {
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(in:r[0][3]) final(cutoff_test)
      {
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(in:r[0][4]) final(cutoff_test)
      {
  ...
  if (r->father == (TNODEPTR)NULL) {
    ...
    *root = nr;
    ...
    if (++f->nsons == 4) {
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(out:root) depend(inout:f) final(cutoff_test)
      {
      taddson(root, f);
    }
    #pragma omp taskwait
  ...
  if (f != (TNODEPTR)NULL) {
    if (f->son1 == v) {
      ...
      if (f->son3 == (TNODEPTR)NULL) {
        {
        cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
        #pragma omp task untied default(shared) depend(inout:f) final(cutoff_test)
        {
        tpatch(f, value);
      }
       }
    ...
    if (f->father->son1 == f) {
      ...
      if (f->father->nsons == 2) {
        if (g->nsons == 2) {
          ...
          {
          cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
          #pragma omp task untied default(shared) depend(inout:root,f) final(cutoff_test)
          {
          tsubson(root, f);
        }
         }
