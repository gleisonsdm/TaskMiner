      ...
      if (ptr->softflag == 1) {
        for (m = 1; m <= ptr->numsites; m++) {
          if (ptr->config[0]->siteLocArray[m].xpos == xpos) {
		     ...
		     */
            hit = 1;
            distant = ABS(ptr->config[0]->siteLocArray[m].ypos -
                          ypos);
            for (m++; m <= ptr->numsites; m++) {
              if (ptr->config[0]->siteLocArray[m].xpos ==
                  xpos) {
                if (ABS(ptr->config[0]->siteLocArray[m].ypos -
                        ypos) < distant) {
                  distant = ABS(ptr->config[0]->siteLocArray[m].ypos -
                                ypos);
                } else {
		     ...
		     */
            hit = 1;
            distant = ABS(ptr->config[0]->siteLocArray[m].xpos -
                          xpos);
            for (m++; m <= ptr->numsites; m++) {
              if (ptr->config[0]->siteLocArray[m].ypos ==
                  ypos) {
                if (ABS(ptr->config[0]->siteLocArray[m].xpos -
                        xpos) < distant) {
                  distant = ABS(ptr->config[0]->siteLocArray[m].xpos -
                                xpos);
                } else {
