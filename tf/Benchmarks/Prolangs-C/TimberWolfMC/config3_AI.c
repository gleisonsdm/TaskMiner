...
void config3(void) {
  ...
  for (cell = 1; cell <= numcells; cell++) {
    ...
    l = cellptr->xcenter + cellptr->config[cellptr->orient]->left - (2 * cellptr->config[cellptr->orient]->lborder);
    r = cellptr->xcenter + cellptr->config[cellptr->orient]->right + (2 * cellptr->config[cellptr->orient]->rborder);
    b = cellptr->ycenter + cellptr->config[cellptr->orient]->bottom - (2 * cellptr->config[cellptr->orient]->bborder);
    t = cellptr->ycenter + cellptr->config[cellptr->orient]->top + (2 * cellptr->config[cellptr->orient]->tborder);
    if (l < left) {
      left = l;
    }
    if (r > right) {
