...
void break_ties(TEAMS_STATS info, ORDERING rank, int within, BOOL display) {
  ...
  for (i = 1; i <= NUM_TEAMS; i++)
    if (!((i == NUM_TEAMS) ? NO : (tied(info, rank[i], rank[i + 1]) ? (within == DIVISION ? (divisions[rank[i]][DIVISION] == divisions[rank[i + 1]][DIVISION] &&
                                                                                             divisions[rank[i]][CONFERENCE] == divisions[rank[i + 1]][CONFERENCE])
                                                                                          : (within == CONFERENCE ? (divisions[rank[i]][CONFERENCE] == divisions[rank[i + 1]][CONFERENCE] && (a_champ(rank[i]) == a_champ(rank[i + 1])))
