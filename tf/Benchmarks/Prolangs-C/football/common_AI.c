...
int head_to_head(TEAM *teams, int num) {
  ...
  for (i = 0; i < num; i++) {
    ...
    pct[i] = (wins + loses + ties == 0 ? 0.0 : ((double)wins + 0.5 * ties) / ((double)(wins + loses + ties)));
    if (pct[i] > best_pct) {
      best_pct = pct[i];
      num_best = 0;
    }
...
void break_a_tie(TEAM *teams, int num, BOOL display) {
  ...
  if (ALL_SAME_DIV || num == 2) {
    ...
    if (value != num) {
      ...
      #pragma omp task untied default(shared) depend(inout:teams) final(cutoff_test)
      {
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(inout:teams[value]) final(cutoff_test)
      {
