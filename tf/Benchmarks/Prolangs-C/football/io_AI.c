...
#define find_conf_rank(r, t) (divisions[t][CONFERENCE] == NFC_0 ? find_teams_rank(r, t) : find_teams_rank(r, t) - NUM_TEAMS / 2)
...
int str_to_int(char *s) {
  ...
  for (temp = s; *temp != '\0'; temp++) {
    value = value * 10 + (((*temp >= '0') && (*temp <= '9')) ? (*temp) - '0' : 0);
  }
  return value;
}
...
void display_rankings(FILE *output) {
  ...
  for (t = 1; t <= NUM_TEAMS / 2; t++) {
    (void)fprintf(output, "%-14s%1s %2d/%-2d %2d/%-2d %2d/%-2d %2d/%-2d %-14s%1s %2d/%-2d %2d/%-2d %2d/%-2d %2d/%-2d\n",
                  ...
                  (a_champ(t + NUM_TEAMS / 2) ? "*" : (a_wild_card(t + NUM_TEAMS / 2) ? "+" : " ")),
