...
void GET_LABEL(LABEL, INPUT, NEXT_CHAR, PASS) char *LABEL; /* Where to put the label if found.               */
  ...
  if ((UPPER(**NEXT_CHAR) >= 'A') && (UPPER(**NEXT_CHAR) <= 'Z')) {
    ...
    if (LABEL_LEN > LABEL_SIZE_1) {
      if (PASS == pass1) {
        ...
        ERROR_MSG = (char *)malloc((unsigned int)(80 + ((int)log10((double)LABEL_SIZE_1)) + strlen(INPUT)));
...
void GET_OPCODE_STR(OPCODE, NEXT_CHAR, PASS, STREAM) char *OPCODE; /* Where to put the opcode that was found.    */
...
FILE *STREAM;                                                      /* Stream to output error messages to.        */
{
  ...
  if ((UPPER(**NEXT_CHAR) >= 'A') && (UPPER(**NEXT_CHAR) <= 'Z')) {
    ...
    int OP_LEN = 1;
    (*NEXT_CHAR)++;
    while (((UPPER(**NEXT_CHAR) >= 'A') && (UPPER(**NEXT_CHAR) <= 'Z')) && (!eoln(**NEXT_CHAR))) {
      OP_LEN++;
      (*NEXT_CHAR)++;
    };
...
void SCAN_LINE(LOCATION_COUNTER, INPUT_LINE, LABEL, EXTENDED, OPERATOR,
               REST, PASS, STREAM, INPUT_STREAM) int LOCATION_COUNTER; /* SIC/XE location counter at start of line*/
