...
make_arg_str(arg_list *args, int len, int commas) {
  ...
  if (args != NULL)
    ...
    #pragma omp task untied default(shared) depend(in:args[0][1]) final(cutoff_test)
    {
...
find_id(id_rec *tree, char *id) {
  ...
  if (cmp_result == 0)
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:tree[0][5]) depend(inout:id) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:tree[0][6]) depend(inout:id) final(cutoff_test)
    {
...
int insert_id_rec(id_rec **root, id_rec *new_id) {
  ...
  if (strcmp(new_id->id, (*root)->id) < 0) {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(inout:root[0][5],new_id) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(inout:root[0][6],new_id) final(cutoff_test)
    {
