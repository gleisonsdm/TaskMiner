...
void calculate_macro_xs(double p_energy, int mat, long n_isotopes,
  ...
  for (int j = 0; j < num_nucs[mat]; j++) {
    ...
    #pragma omp task untied default(shared) depend(in:p_nuc,energy_grid,nuclide_grids)
    {
    calculate_micro_xs(p_energy, p_nuc, n_isotopes, n_gridpoints, energy_grid, nuclide_grids, idx, xs_vector);
    }
     }
    for (int k = 0; k < 5; k++)
