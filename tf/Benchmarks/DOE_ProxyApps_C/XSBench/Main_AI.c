...
// Prepare Unionized Energy Grid Framework
  ...
  int *index_data = (int *)malloc(in.n_isotopes * in.n_gridpoints * in.n_isotopes * sizeof(int));
...
#ifdef OPENMP
#pragma omp parallel default(none) private(i, thread, p_energy, mat, seed)                       \
      ...
      for (i = 0; i < in.lookups; i++) {
        ...
        if (INFO && mype == 0 && thread == 0 && i % 1000 == 0)
          printf("\rCalculating XS's... (%.0lf%% completed)",
                 (i / ((double)in.lookups / (double)in.nthreads)) / (double)in.nthreads * 100.0);
