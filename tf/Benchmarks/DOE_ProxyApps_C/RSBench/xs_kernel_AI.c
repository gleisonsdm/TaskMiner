...
void calculate_macro_xs(double *macro_xs, int mat, double E, Input input, CalcDataPtrs data, complex double *sigTfactors, long *abrarov, long *alls) {
  ...
  for (int i = 0; i < (data.materials).num_nucs[mat]; i++) {
    ...
    if (input.doppler == 1)
      ...
      #pragma omp task untied default(shared) depend(in:nuc,input,data) depend(inout:sigTfactors,abrarov,alls)
      {
      ...
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(in:nuc,input,data) depend(inout:sigTfactors)
      {
