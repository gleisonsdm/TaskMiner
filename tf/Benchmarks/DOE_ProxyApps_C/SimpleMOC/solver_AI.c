...
// print progress
      ...
      if (I->mype == 0 && thread == 0) {
        printf("\rAttenuating Tracks... (%.0lf%% completed)",
               (i / ((double)I->ntracks_2D / (double)nthreads)) / (double)nthreads * 100.0);
      }
#else
      if (i % 50 == 0)
        if (I->mype == 0)
...
void alt_attenuate_fluxes(Track *track, bool forward, Source *QSR, Input *I,
  ...
  for (int g = 0; g < I->n_egroups; g++) {
    ...
    // add contribution to new source flux
    float flux_integral = (q0 * tau + (sigT * psi[g] - q0) * expVal) / sigT2 + q1 * mu * (tau * (tau - 2) + 2 * expVal) / (sigT * sigT2) + q2 * mu2 * (tau * (tau * (tau - 3) + 6) - 6 * expVal) / (3 * sigT2 * sigT2);

#pragma omp atomic
    FSR_flux[g] += weight * flux_integral;

    // update angular flux
    psi[g] = psi[g] * (1.0 - expVal) + q0 * expVal / sigT + q1 * mu * (tau - expVal) / sigT2 + q2 * mu2 * (tau * (tau - 2) + 2 * expVal) / (sigT2 * sigT);
  }
}
...
float update_sources(Params params, Input I, float keff) {
  ...
  float *residuals = (float *)malloc(I.n_source_regions_per_node * sizeof(float));
  ...
  for (long i = 0; i < I.n_source_regions_per_node; i++) {
    ...
    for (int j = 0; j < I.fai; j++) {
      ...
      for (int g = 0; g < I.n_egroups; g++) {
        ...
        group_res[g] = (newSrc - oldSrc) * (newSrc - oldSrc) / (oldSrc * oldSrc);
