...
// Allocate space for ALL scattering data
...
#endif
  float *s_matrix_data = (float *)malloc(n_xs_regions * I.n_egroups * I.n_egroups * sizeof(float));
  ...
  *nbytes += n_xs_regions * I.n_egroups * sizeof(float *);
  ...
  float *XS_data = (float *)malloc(n_xs_regions * I.n_egroups * 3 * sizeof(float));
  *nbytes += n_xs_regions * I.n_egroups * 3 * sizeof(float);
  ...
  float ***fineSource = (float ***)malloc(I.n_source_regions_per_node * sizeof(float **));
  *nbytes += I.n_source_regions_per_node * sizeof(float **);

  // Allocate space for array pointers to parameters
  float **fineSourcePtrs = (float **)malloc(I.n_source_regions_per_node * I.fai * sizeof(float *));
  *nbytes += I.n_source_regions_per_node * I.fai * sizeof(float *);

  // Allocate space for flux parameters (quadratic axially)
  float ***fineFlux = (float ***)malloc(I.n_source_regions_per_node * sizeof(float **));
  *nbytes += I.n_source_regions_per_node * sizeof(float **);

  // Allocate space for array pointers to parameters
  float **fineFluxPtrs = (float **)malloc(I.n_source_regions_per_node * I.fai * sizeof(float *));
  *nbytes += I.n_source_regions_per_node * I.fai * sizeof(float *);

  // Allocate space for cross section pointers
  float **sigT = (float **)malloc(I.n_source_regions_per_node * sizeof(float *));
  ...
  for (long i = 0; i < I.n_source_regions_per_node; i++)
    for (int j = 0; j < I.fai; j++)
      fineFlux[i][j] = &data[I.n_egroups * I.fai *
                                 (I.n_source_regions_per_node + i) +
                             j * I.n_egroups];

  // stitch allocation ptrs together for total XS
  for (long i = 0; i < I.n_source_regions_per_node; i++)
    sigT[i] = &data[2 * I.n_source_regions_per_node * I.fai * I.n_egroups + i * I.n_egroups];
