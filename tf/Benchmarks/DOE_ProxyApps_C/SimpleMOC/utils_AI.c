...
float pairwise_sum(float *vector, long size) {
  ...
  if (size <= 16)
    ...
    #pragma omp task untied default(shared) depend(in:vector) final(cutoff_test)
    {
    sum = pairwise_sum(&vector[0], size / 2) +
          ...
          long long int TM2[1];
          TM2[0] = size / 2;
          #pragma omp task untied default(shared) depend(in:vector[TM2[0]]) final(cutoff_test)
          {
...
size_t est_mem_usage(Input I) {
  ...
  nbytes += I.ntracks_2D * I.n_polar_angles * z_stacked * I.n_egroups * sizeof(float) * 2;
  ...
  nbytes += I.nthreads * I.z_stacked * sizeof(int);
  nbytes += I.nthreads * I.z_stacked * 2 * I.segments_per_track * sizeof(double);
  nbytes += I.nthreads * I.z_stacked * 2 * I.segments_per_track * sizeof(Source *);
