...
void TelescopingCABiCGStab(domain_type *domain, int level, int e_id, int R_id, double a, double b, double desired_reduction_in_norm) {
  ...
  while ((m < mMax) && (!BiCGStabFailed) && (!BiCGStabConverged)) { // while(not done){
    ...
    for (i = 0; i < 4 * __ca_krylov_s + 1; i++) {
      PRrt[i] = __PRrt + i;
    }                                      // columns of PRrt map to the consecutive spare grid indices starting at __PRrt
    ...
    for (n = 0; n < __ca_krylov_s; n++) {                                                    // for(n=0;n<__ca_krylov_s;n++){
      ...
      if (L2_norm_of_s < 0)
        ...
        L2_norm_of_s = sqrt(L2_norm_of_s); // finite precision can lead to the norm^2 being < 0 (Demmel says flush to 0.0)
#ifdef __VERBOSE                           //
      if (domain->rank == 0) {
      ...
      if (cj_dot_Gcj > 0)
        L2_norm_of_residual = sqrt(cj_dot_Gcj); // finite precision can lead to the norm^2 being < 0 (Demmel says flush to 0.0)
#ifdef __VERBOSE
      if (domain->rank == 0) {
...
void CACG(domain_type *domain, int level, int e_id, int R_id, double a, double b, double desired_reduction_in_norm) {
  ...
  for (i = 0; i < 2 * __CA_KRYLOV_S + 1; i++) {
    PR[i] = __PRrt + i;
  } // columns of PR map to the consecutive spare grids allocated for the bottom solver starting at __PRrt
  ...
  while ((m < mMax) && (!CGFailed) && (!CGConverged)) { // while(not done){
    ...
    for (n = 0; n < __CA_KRYLOV_S; n++) {                                                    // for(n=0;n<__CA_KRYLOV_S;n++){
      ...
      L2_norm_of_residual = 0.0;
      if (cj_dot_Gcj_new > 0)
        L2_norm_of_residual = sqrt(cj_dot_Gcj_new); // finite precision can lead to the norm^2 being < 0 (Demmel says flush to 0.0)
