...
static int nearu(p_type p, int maxdep, int dep, int last, int near[]) {
  ...
  if (last != 0)
    neard(m0u(p), maxdep, dep + 1, 0, near);
  if (last != 1)
    neard(m1u(p), maxdep, dep + 1, 1, near);
  if (last != 2)
    neard(m2u(p), maxdep, dep + 1, 2, near);
...
static int neard(p_type p, int maxdep, int dep, int last, int near[]) {
  if (last != 0)
    nearu(m0d(p), maxdep, dep + 1, 0, near);
  if (last != 1)
    nearu(m1d(p), maxdep, dep + 1, 1, near);
  if (last != 2)
    nearu(m2d(p), maxdep, dep + 1, 2, near);
  return 0;
}
