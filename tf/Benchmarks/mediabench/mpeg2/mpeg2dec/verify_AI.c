...
void Check_Headers(Bitstream_Framenum, Sequence_Framenum) int Bitstream_Framenum;
...
static void Check_VBV_Delay(Bitstream_Framenum, Sequence_Framenum) int Bitstream_Framenum;
int Sequence_Framenum;
{
  ...
  if ((Sequence_Framenum == 0) && (!Second_Field)) {/* first coded picture of sequence */
      ...
      if (picture_coding_type == B_TYPE) {
        ...
        if (picture_structure == FRAME_PICTURE) {
          ...
          if (Second_Field == 0) /* first field of current frame */
            ...
            if (previous_IorP_picture_structure != FRAME_PICTURE || previous_IorP_repeat_first_field == 0)
              I = 2 * T - T; /* a net of one field period */
            else if (previous_IorP_picture_structure == FRAME_PICTURE && previous_IorP_repeat_first_field == 1)
