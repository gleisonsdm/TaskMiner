...
void Substitute_Frame_Buffer(bitstream_framenum, sequence_framenum) int bitstream_framenum;
  ...
  if ((sequence_framenum != 0) || (Second_Field)) {
    ...
    if ((picture_structure == FRAME_PICTURE) || (!Second_Field)) {
      ...
      /* only the first B frame in a consequitve set of B pictures
        ...
        substitute_display_framenum =
            (previous_temporal_reference - temporal_reference) + bitstream_framenum - 1;
      ...
      if ((previous_picture_coding_type == I_TYPE) && (picture_coding_type == P_TYPE)) {
        substitute_display_framenum = bitstream_framenum;
          ...
          {
        substitute_display_framenum =
            (temporal_reference - previous_anchor_temporal_reference) + bitstream_framenum - 1;
      }
...
static void Copy_Frame(src, dst, width, height, parity, field_mode) unsigned char *src;
unsigned char *dst;
int width;
int height;
