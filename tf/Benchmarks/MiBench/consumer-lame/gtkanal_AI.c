...
int gtkmakeframe(void) {
  ...
  if (gfp->input_format == sf_mp3) {
    ...
    while (gfp->frameNum == pinfo->frameNum) {
      ...
      if (gfp->frameNum == 1)
        init = 0; /* reset for next time frameNum==0 */
      iread = lame_readframe(gfp, Buffer);
...
void plot_frame(void) {
  ...
  for (i = 0; i < n; i++) {
    xcord[i] = i;
    if (gtkinfo.msflag)
      ycord[i] = ch ? .5 * (pplot->pcmdata[0][i] - pplot->pcmdata[1][i]) : .5 * (pplot->pcmdata[0][i] + pplot->pcmdata[1][i]);
    else
  ...
  */
  ...
  for (j = 1152 - n, i = 0; i <= n; i++, j++) {
    xcord[i] = i;
    if (gtkinfo.msflag)
      ycord[i] = ch ? .5 * (pplot1->pcmdata2[0][j] - pplot1->pcmdata2[1][j]) : .5 * (pplot1->pcmdata2[0][j] + pplot1->pcmdata2[1][j]);
    else
  ...
  for (i = 0; i < n; i++) {
    xcord[i] = i + 224;
    if (gtkinfo.msflag)
      ycord[i] = ch ? .5 * (pplot2->pcmdata2[0][i] - pplot2->pcmdata2[1][i]) : .5 * (pplot2->pcmdata2[0][i] + pplot2->pcmdata2[1][i]);
    else
  ...
  if (idle_keepgoing) {
	...
	*/
        ...
        if (pinfo->frameNum > 0) /* start averaging at second frame */
          gtkinfo.avebits = (gtkinfo.avebits * ((pinfo->frameNum) - 1) + pinfo->totbits) / (pinfo->frameNum);
