      ...
      for (band = 0; band < 32; band++, mdct_enc += 18) {
        ...
        if (band >= gfp->lowpass_band || band <= gfp->highpass_band) {
          ...
          if (type == SHORT_TYPE) {
            ...
            for (k = 8; k >= 0; --k) {
              work[k] =
                  win[type][k] * sb_sample[ch][gr][k][band] - win[type][k + 9] * sb_sample[ch][gr][17 - k][band];

              work[9 + k] =
                  win[type][k + 18] * sb_sample[ch][1 - gr][k][band] + win[type][k + 27] * sb_sample[ch][1 - gr][17 - k][band];
            }
