...
#define rpelev2 16
  ...
  for (chn = 0; chn < numchn; chn++) {
     ...
     **********************************************************************/
    for (j = 0; j < cw_lower_index; j++) { /* calculate unpredictability measure cw */
      FLOAT an, a1, a2;
      FLOAT bn, b1, b2;
      FLOAT rn, r1, r2;
     ...
     **********************************************************************/
    for (j = cw_lower_index; j < cw_upper_index; j += 4) { /* calculate unpredictability measure cw */
      FLOAT rn, r1, r2;
      FLOAT numre, numim, den;
    ...
    for (b = 0; b < npart_l; b++) {
      ...
      if (tbb != 0) {
        ...
        if (tbb <= 0.04875584301) {
          ...
          tbb = log(tbb);
          tbb = exp(((TMN - NMT) * (LN_TO_LOG10 * 0.299)) + ((TMN - NMT) * (LN_TO_LOG10 * 0.43)) * tbb); /* conv1=-0.299, conv2=-0.43 */
        }
      }
      ...
      if (gfp->no_short_blocks) {
        ...
        if (pe[chn] > 3000) {
          ...
          if (mx > 30 * mn) { /* big surge of energy - always use short blocks */
            uselongblock[chn] = 0;
          } else if ((mx > 10 * mn) && (pe[chn] > 1000)) { /* medium surge, medium pe - use short blocks */
            uselongblock[chn] = 0;
          }
        }
      }
