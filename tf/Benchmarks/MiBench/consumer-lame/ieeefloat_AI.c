...
ConvertFromIeeeSingle(char *bytes) {
  ...
  bits = ((unsigned long)(bytes[0] & 0xFF) << 24) | ((unsigned long)(bytes[1] & 0xFF) << 16) | ((unsigned long)(bytes[2] & 0xFF) << 8) | (unsigned long)(bytes[3] & 0xFF); /* Assemble bytes into a long */
...
void main(void) {
  ...
  d[0] = 0x00000001L;
  MaybeSwapBytes(d, 4);
  TestToIeeeSingle(*((float *)(&d[0])) * 0.5); /* Smaller than smallest denorm */
  d[0] = 0x3F8FEDCBL;
  MaybeSwapBytes(d, 4);
  ...
  MaybeSwapBytes(d, 8);
  TestToIeeeDouble(*((Double *)(bytes))); /* Normalized, problem with unsigned */
  ...
  Hex2Bytes("0000000000000001", bytes);
  MaybeSwapBytes(d, 8);
  TestToIeeeDouble(*((Double *)(bytes)) * 0.5); /* Smaller than smallest denorm */
