...
insertR(struct ptree *h, struct ptree *n, int d, struct ptree *p) {
  ...
  if (bit(h->p_b, n->p_key))
    ...
    #pragma omp task untied default(shared) depend(inout:h[0].p_right,n,h) final(cutoff_test)
    {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(inout:h[0].p_left,n,h) final(cutoff_test)
    {
