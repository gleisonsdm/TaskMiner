...
void cilksort(ELM *low, ELM *tmp, long size) {
  ...
  {
  cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
  #pragma omp task untied default(shared) depend(inout:A,tmpA) final(cutoff_test)
  {
  ...
  {
  cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
  #pragma omp task untied default(shared) depend(inout:B,tmpB) final(cutoff_test)
  {
  ...
  {
  cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
  #pragma omp task untied default(shared) depend(inout:C,tmpC) final(cutoff_test)
  {
  ...
  {
  cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
  #pragma omp task untied default(shared) depend(inout:D,tmpD) final(cutoff_test)
  {
...
void sort_init(void) {
  ...
  if (bots_app_cutoff_value < 2) {
    ...
    printf("%s can not be greather than vector size, using %d as a parameter.\n", BOTS_APP_DESC_ARG_CUTOFF, bots_arg_size);
