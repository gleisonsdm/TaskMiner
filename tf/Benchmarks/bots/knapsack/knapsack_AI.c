...
void knapsack(struct item *e, int c, int n, int v, int *sol) {
  ...
  #pragma omp task untied default(shared) depend(in:e[1]) shared(without) final(cutoff_test)
  {
  ...
  {
  cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
  #pragma omp task untied default(shared) depend(in:e[1]) shared(with) final(cutoff_test)
  {
