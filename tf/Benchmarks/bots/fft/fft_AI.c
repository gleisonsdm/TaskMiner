...
void compute_w_coefficients(int n, int a, int b, COMPLEX *W) {
  ...
  if (b - a < 128) {
    ...
    #pragma omp task untied default(shared) depend(out:W) final(cutoff_test)
    {
...
void unshuffle(int a, int b, COMPLEX *in, COMPLEX *out, int r, int m) {
  ...
  if (b - a < 16) {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(inout:in,out) final(cutoff_test)
    {
...
void fft_twiddle_gen(int i, int i1, COMPLEX *in, COMPLEX *out, COMPLEX *W, int nW, int nWdn, int r, int m) {
  ...
  if (i == i1 - 1) {
    ...
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:in,W) depend(out:out) final(cutoff_test)
    {
...
void fft_aux(int n, COMPLEX *in, COMPLEX *out, int *factors, COMPLEX *W, int nW) {
  ...
  if (r < n) {
    ...
    for (k = 0; k < n; k += m) {
      {
      cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
      #pragma omp task untied default(shared) depend(in:factors[1],W) depend(inout:out[k],in[k]) final(cutoff_test)
      {
