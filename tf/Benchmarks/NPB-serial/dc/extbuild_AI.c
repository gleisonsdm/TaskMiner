...
int32 SharedSortAggregate(ADC_VIEW_CNTL *avp) {
  ...
  for (iib = 1; iib <= nsgs; iib++) {
    if (iib > 1)
      FSEEK(avp->viewFile, inpfOffset, SEEK_SET);
    if (iib == nsgs)
      ncur = nlst;
    else
      ncur = nreg;
...
int32 RunFormation(ADC_VIEW_CNTL *avp, FILE *inpf) {
  ...
  for (iRec = 1; iRec <= avp->nRowsToRead; iRec++) {
    ...
    #pragma omp task untied default(shared) depend(inout:avp[0][40],avp[0][9],avp[0][8],avp[0][10])
    {
...
int32 MultiWayMerge(ADC_VIEW_CNTL *avp) {
  ...
  for (i = 0; i < avp->numberOfChunks; i++) {
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(inout:chunkRecSize,multiChunkBuffer,avp[0][24],avp[0][35])
    {
    ReadSubChunk(
        }
         }
