...
taskminer_depth_cutoff++;
{
  if (list != NULL) {
    ...
    #pragma omp task untied default(shared) depend(in:list[0].next) final(cutoff_test)
    {
    ...
    struct _play *play;
{
  if (play != NULL) {
    {
    cutoff_test = (taskminer_depth_cutoff < DEPTH_CUTOFF);
    #pragma omp task untied default(shared) depend(in:play[0].next) final(cutoff_test)
    {
