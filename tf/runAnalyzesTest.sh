...
$OPT -load $ST -load $PTRA -load $WTM -load $WAI -stats -instnamer -mem2reg \
   -writeInFile -Run-Mode=true -debug-only=print-tasks \
   -S result.bc -o result2.bc &> /tmp/tempao.txt

bash $HOME/tf/processTempao.sh
